...
@jwt_required()
def get_balance():
    """Get current wallet balance (backed by Firestore users/{uid}.deposit)."""
    try:
...
def add_funds():
    ...
    try:
        ...
        if amount is None:
            return (
                jsonify({"success": False, "error": "Amount required"}),
                400,
            )
...
@jwt_required()
def process_refund():
    """Process refund (manager only – caller must enforce role)."""
    try:
        data = request.get_json() or {}
        ...
        if user_id is None or order_id is None or amount_raw is None:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "user_id, order_id, and amount are required",
                    }
                ),
                400,
            )
        ...
        wallet, transaction = FinanceService.process_refund(
